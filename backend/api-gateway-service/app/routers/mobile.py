...
from app.utils.dependencies import get_optional_user, get_current_user
...
@router.get("/products/history", response_model=schemas.HistoryResponse)
async def get_scan_history(
    ...
    """
    Récupère l'historique des scans de l'utilisateur authentifié
    
    Retourne la liste des jobs terminés avec leurs scores
    """
    from app.models import JobStatus
    
    # Récupérer les jobs de l'utilisateur, triés par date de création (plus récent en premier)
    ...
    for job in jobs:
        ...
        if job.result:
            score_letter = job.result.get("score_letter")
            score_value = job.result.get("score_value")
        
        # Essayer d'extraire le nom du produit depuis parser_result
        if job.parser_result and isinstance(job.parser_result, dict):
            product_name = (
                job.parser_result.get("product_name") or
                job.parser_result.get("name") or
                job.parser_result.get("title")
            )
